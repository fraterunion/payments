...
export async function createTestOrganization(
  ...
  const slug = `events-test-${randomUUID().slice(0, 8)}`;
  const organization = await db.organization.create({
    data: {
      name: `Events Test ${slug}`,
      slug,
      type: 'BUSINESS',
      status: 'ACTIVE',
      defaultCurrency: 'USD',
...
export async function cleanupPlatformEvents(
  db: PrismaClient,
  options: { readonly eventTypePrefix?: string; readonly sourcePrefix?: string },
): Promise<void> {
  if (options.eventTypePrefix !== undefined) {
    await db.outboxEvent.deleteMany({
      where: { organizationId: null, eventType: { startsWith: options.eventTypePrefix } },
    });
  }
  if (options.sourcePrefix !== undefined) {
    await db.inboxEvent.deleteMany({
      where: { organizationId: null, source: { startsWith: options.sourcePrefix } },
    });
  }
}
